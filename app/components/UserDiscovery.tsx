...
const UserDiscovery = memo(function UserDiscovery({
  ...
  const [allUsers, setAllUsers] = useState<
    ...
    Array<{
      ...
      badges?: Array<{ earned: boolean; xpReward?: number }>;
  ...
  const handleUserClick = useCallback(
    ...
    (username: string, fallbackId?: string | number) => {

      measure("navigate-to-profile", () => {

        // Use username if available, otherwise fallback to id

        const profileIdentifier = username || fallbackId || 'unknown';
  ...
  const getRankIcon = useCallback((rank: string) => {
    ...
    if (rankLower.includes("creative"))
  ...
  const UserCard = memo(function UserCard({
    ...
    }, [user.username, user.id, handleUserClick]);
  ...
  return (
    ...
    <div ref={contentRef} className="w-full max-w-4xl mx-auto">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-white mb-2 flex items-center gap-2 bg-linear-to-r from-blue-400 to-purple-400 bg-clip-text">
      ...
      <div
        ...
        className={`relative mb-6 transform transition-all duration-700 ease-out ${
      ...
      >
        ...
        <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400  transition-transform duration-300 hover:scale-110" />
      ...
      {!loading && (
        ...
        <div
          ...
          className={`mt-8 pt-6 border-t border-gray-800 transform transition-all duration-700 ease-out ${
