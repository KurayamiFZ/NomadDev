...
export function AchievementDemo() {
  ...
  const { unlockAchievement, getUserBadges, getAllAchievements, initialize } = useAchievements();
  ...
  return (
    <div className="bg-gray-900 rounded-lg p-6 border border-gray-800 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="text-2xl font-bold text-white">Achievement System Demo</h2>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold text-purple-400">
              {Math.round((badges.filter(b => b.earned).length / badges.length) * 100) || 0}%
            </div>
      ...
      {/* Demo Actions */}
      <div className="mb-6">
        <h3 className="text-lg font-bold text-white mb-4">Test Achievement Unlocking</h3>
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-xl ${
                  ...
                  {badge.earned ? (badge.icon || "🏆") : <Lock className="w-5 h-5 text-gray-500" />}
                </div>
                <div className="flex-1">
                  <h4 className={`font-bold ${badge.earned ? "text-white" : "text-gray-400"}`}>
                    {badge.title}
                  </h4>
              ...
              <div className="text-xs text-gray-400">
                ...
                {badge.earned && badge.xpReward && (
                  <span className="ml-2 text-yellow-400">+{badge.xpReward} xp</span>
                )}
              </div>
            </div>
          ))}
      ...
      {badges.length === 0 && (
        <div className="text-center py-8">
          ...
          <div className="text-sm text-gray-500">Make sure you're logged in and have achievements available</div>
        </div>
      )}
    </div>
  );
